...
public class Persistencia {
	...
	private static List<Usuario> lista2 = new ArrayList<Usuario>(0);
	...
	static {
		try {
			Empresa empresa1 = new Empresa(UUID.randomUUID(), "Supervi", "88888888888", sdf.parse("01/01/2022"));
			Empresa empresa2 = new Empresa(UUID.randomUUID(), "Atende+", "98585874587", sdf.parse("01/05/2022"));
			Empresa empresa3 = new Empresa(UUID.randomUUID(), "Hipervip", "98585874587", sdf.parse("01/12/2021"));
			lista.add(empresa1);
			lista.add(empresa2);
			lista.add(empresa3);

			Usuario usuario1 = new Usuario("admin", "1234");
			Usuario usuario2 = new Usuario("user", "1234");
			lista2.add(usuario1);
			lista2.add(usuario2);
	...
	public void adicionarUsuario(Usuario objUsuario) {
		Persistencia.lista2.add(objUsuario);
	}
	
	public Usuario buscarUsuarioPorLogin(String login) {
		for (Usuario u : Persistencia.lista2)
			if (u.getLogin().equals(login))
				return u;
		return null;
	}

	public List<Usuario> obterTodosUsuarios(Usuario objUsuario) {
		return Persistencia.lista2;
	}

	public void adicionarEmpresa(Empresa objEmpresa) {
	...
	public void atualizarEmpresa(Empresa objEmpresa) {
		Iterator<Empresa> it = Persistencia.lista.iterator();
		while (it.hasNext()) {
			Empresa e = it.next();
